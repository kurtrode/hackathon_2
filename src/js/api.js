...
const getData = async () => {
  const getFetch = await fetch(
    "https://test-api.codingbootcamp.cz/api/21c8af4d/events"
  );
...
const clearEverything = async () => {
  const getClear = await fetch(
    "https://test-api.codingbootcamp.cz/api/21c8af4d/events/reset",
...
const registerData = async () => {
  ...
  const myRegister = await fetch(url, {
    method: "POST",
    headers: { "Content-type": "application/json" },
    body: JSON.stringify(nameAndStuff),
  });
